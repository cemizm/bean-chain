...
module.exports = class CCIParser {
    constructor(port) {
        ...
        this.port.on('data', (rawdata) => {
            rawdata.forEach((value) => this._handleInput(value));
        });
    }
    ...
    setPrice(pricelist, product, price) {
        if(!this.prices[pricelist])
            return this.prices[pricelist] = {};
        
        this.prices[pricelist][product] = price;
    }

    _execCallback(name, request, response){
        if(!this.cbs[name])
            return;

        this.cbs[name](request, response);
    }
    ...
    _handleTelegram(data) {
        ...
        switch(type){
            ...
            case TelegramTypes.Credit:
                result = this.handleCredit(payload);
                break;
    ...
    handleInquiry(payload) {
        ...
        this._execCallback('inquiry', request, response);
    ...
    handleVend(payload) {
        let request = {
            mode: payload.toString('ascii', 0, 1)
        }
